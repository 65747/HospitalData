...
Les noms des propriétés en C# sont alignés sur les clés JSON
Voici les structures utilisées côté fichier (et reflétées en mémoire).
...
SessionsManager (Hospital.Data.Storage)
    ...
    {    
        ...
        {    
            ...
            public SessionsManager(PatientsManager patientsManager, SuperviseursManager superviseursManager, string path = "Data/sessions.json");    
...
HospitalDataService — point d’accès unique
         
    namespace Hospital.Data.Storage    
    {    
        public class HospitalDataService    
        {    
            private static HospitalDataService _instance;    
            private readonly string _basePath;    
            private PatientsManager _patients;    
            private SuperviseursManager _superviseurs;    
            private SessionsManager _sessions;    
         
            public static HospitalDataService Instance { get; }    
            public string BasePath { get; }    
            public PatientsManager Patients { get; }    
            public SuperviseursManager Superviseurs { get; }    
...
3.2 Afficher la liste complète des patients 
...
GetAll() retourne une copie de la liste ; modifier les éléments ne modifie pas le fichier tant qu’on n’appelle pas Update ou Add.
...
Lors d’un `Patients.Remove(dASupprimer)`, le service déclenche en interne la suppression de toutes les sessions dont `IDpatient` correspond, puis met à jour `sessions.json`. Un seul appel suffit
