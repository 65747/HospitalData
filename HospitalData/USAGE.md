...
| `GetById(string id)` | Un patient par ID (insensible à la casse), sinon `null` |
...
| `Remove(string idPatient)` | Supprimer un patient par ID. Retourne `true` si supprimé. |
...
// Recherche par ID
var patient = data.Patients.GetById("patient-001");
if (patient != null)
    Debug.Log(patient.Pathologie);
...
| `GetById(string id)` | Un par ID, sinon `null` |
...
var sup = data.Superviseurs.GetById("sup-123");
if (sup != null)
    data.Superviseurs.Remove(sup.IdSuperviseur);
